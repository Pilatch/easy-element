...
// or blows up.
module.exports = (preprocessor, stylesText) => {
  if (preprocessor === 'postcss') {
    return preprocessPostcss(stylesText)
  } else {
...
let preprocessPostcss = stylesText => {
  ...
  if (postcssConfigFile) {
    try {
      ...
      if (config && config.plugins) {
        return require('postcss')(config.plugins)
          .process(stylesText, {from: undefined})
          .catch(error => {
    ...
    console.error('Could not find "postcss.config.js" in any parent directory.')
    process.exit(1)
  }
}