...
// or blows up.
module.exports = (preprocessor, stylesText, cssInput) => {
  if (preprocessor === 'postcss') {
    return preprocessPostcss(stylesText, cssInput)
  } else {
...
let preprocessPostcss = (stylesText, cssInput) => {
    ...
    try {
      ...
      if (config && config.plugins) {
        return require('postcss')(config.plugins)
          .process(stylesText, {from: cssInput})
          .catch(error => {
