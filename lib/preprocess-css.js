...
module.exports = (preprocessor, stylesText, input) => {
  ...
  if (preprocessor === 'sass') {
    return preprocessSass(stylesText, input)
  }
...
let preprocessSass = (stylesText, input) => {
    ...
    try {
      ...
      if (stylesText) {
        result = sass.renderSync({
          data: stylesText,
          includePaths: [path.dirname(input)],
        })
      } else {
        result = sass.renderSync({file: input})
      }

      resolve(result.css.toString('utf8'))
    } catch (error) {
      console.error(`While processing SASS, ${error}`)
      process.exit(1)
    }
  })
}
