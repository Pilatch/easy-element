...
let getElementClasses = (ast, className) => astQuery.query(ast, '// ClassDeclaration').filter(
  bodyObject => bodyObject.id.name === className
)
...
let getConstructors = (ast, elementClass) =>
...
let mutateConstructor = (ast, className) => {
  ...
  if (elementClass) {
    let cons = getConstructors(ast, elementClass)
