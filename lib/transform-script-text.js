...
let getConstructors = (ast, className) => (
  getElementClasses(ast, className)[0].body.body.filter(
...
let mutateConstructor = (ast, className) => {
  let cons = getConstructors(ast, className)
...
module.exports = (scriptText, tagName, className, innerHTML, stylesText, minify) => {
  ...
  if (ast && ast.program && ast.program.body) {
    ...
    if (stylesText) {
      es5Ast.program.body.push(appendStyleAst(minifyCss(elementNameStyles(tagName, stylesText), minify)))
    }
    ...
    mutateES6ConnectedCallback(es6Ast, className, innerHTML, minifyCss(shadowDomStyles(tagName, stylesText), minify))
