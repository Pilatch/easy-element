...
let shouldTransformSelector = (tagName, selector) => {
  if (selector === tagName) {
    return true
  }

  let trailingChar = selector.charAt(tagName.length)

  if (selector.startsWith(tagName) && '# :.'.includes(trailingChar)) {
    return true
  }
...
let shadowDomStyles = (tagName, stylesText) => {
  if (stylesText) {
    ...
    // if (tagName === 'name-tag') console.log('stylesObject', JSON.stringify(stylesObject, null, '  '))
    stylesObject.stylesheet.rules.forEach(rule => {
      rule.selectors = rule.selectors.map(selector => {
        console.log(`shouldTransformSelector(${tagName}, ${selector})`, shouldTransformSelector(tagName, selector))
        if (shouldTransformSelector(tagName, selector)) {
          return selector.replace(tagName, ':host')
        }
