...
let parseStyles = stylesText => {
  try {
    ...
    console.error(`Error parsing CSS at line ${error.line}, column ${error.column}: ${error.reason}.\n`)
    console.error(chalk.white(errorLines[error.line - 4] || ''))
    console.error(chalk.white(errorLines[error.line - 3] || ''))
    console.error(chalk.white(errorLines[error.line - 2] || ''))
    console.error(chalk.red(errorLines[error.line - 1])) // the offending line
    console.error(chalk.white(errorLines[error.line] || ''))
    console.error(chalk.white(errorLines[error.line + 1] || ''))
    console.error(chalk.white(errorLines[error.line + 2] || ''), '\n')
    process.exit(1)
  }
}
    ...
    stylesObject.stylesheet.rules.forEach(rule => {
      if (rule.selectors) {
        try {
          ...
          console.error(`${selectorsException.message}\nError in CSS selector:\n\n${rule.selectors}\n`)
          process.exit(1)
        }
      }
    })
...
module.exports = (scriptText, tagName, className, outputFolder, innerHTML, stylesText) => {
  ...
  if (ast && ast.program && ast.program.body) {
    ...
    if (connectedCallbacks.length > 1) {
      console.error(`I don't know what to do with more than one connectedCallback for your ${className} custom element class.`)
      process.exit(1)
    }
