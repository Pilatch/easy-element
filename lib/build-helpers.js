...
let buildDirectory = (options, input) => {
  ...
  Promise
    ...
    .then(results => {
      ...
      results.forEach(writeResult(options.outputFolder))
    })
}

let buildFileGroup = (options, group) => transformFileGroup(options, group).then(writeResult(options.outputFolder))
