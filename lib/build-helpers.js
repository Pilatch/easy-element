...
let buildDirectory = (options, input) => {
  ...
  Object.keys(fileGroups).forEach(basename => {
    transformPromises.push(buildFileGroup(options, fileGroups[basename]))
  })
  ...
  Promise
    ...
    .then(results => {
      if (options.bundle) {
        let bundleResult = require('./bundle')(results)

        if (bundleResult) {
          let writeToFile = require('./write-to-file')

          writeToFile.es5(bundleResult.es5Ast, 'bundle', options.outputFolder)
          writeToFile.es6(bundleResult.es6Ast, 'bundle', options.outputFolder)
        }
      } else {
        results.forEach(result => {
          if (result) {
            ...
            writeToFile.es5(result.es5Ast, result.tagName, options.outputFolder)
            writeToFile.es6(result.es6Ast, result.tagName, options.outputFolder)
          }
        })
      }
    })
...
let buildFileGroup = (options, group) => {
  ...
  return require('./css-input')(stylesText, stylesSource, preprocessor || inferredPreprocessor)
...
let writeResult = outputFolder => result => {
  if (result) {
    let writeToFile = require('./write-to-file')

    writeToFile.es5(result.es5Ast, result.tagName, outputFolder)
    writeToFile.es6(result.es6Ast, result.tagName, outputFolder)
  }
}
