...
let mightHaveCssImports = input => input.endsWith('.scss') || input.endsWith('.sass')

// Returns an import map where leaf-nodes map to the parent files
// Returns a Promise
let importsToWatch = (input, inputIsDirectory = false) => {
  if (inputIsDirectory) {
    return recursiveReaddir(input, true)
      .then(directoryContents => directoryContents
        ...
        .reduce(
          (accumulator, importMap) => Object.assign(accumulator, importMap),
          {}
        )
      )
  }
  ...
  if (mightHaveCssImports(input)) {
    // TODO how do we handle regular CSS files for postCSS?
    // TODO how do we handle HTML with a style tag that imports CSS?
...
let _invertImportMap = importMap => Object
  ...
  .reduce((accumulator, parentFile) => {
    ...
    return Object.assign(accumulator, childFiles.reduce((childToAncestorMap, childFile) => {
      childToAncestorMap[childFile] = parentFile
