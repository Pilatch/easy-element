...
let getCssFile = directoryContents => {
  ...
  newMatch = fileInDirectoryByExtension(directoryContents, '.scss')

  if (newMatch) {
    if (cssExtension) {
      fail(`Your input directory has both a ${cssExtension} and a .scss file. Only one styles file is allowed.`)
    }
    cssExtension = '.scss'
  ...
  newMatch = fileInDirectoryByExtension(directoryContents, '.sass')

  if (newMatch) {
    if (cssExtension) {
      fail(`Your input directory has both a ${cssExtension} and a .sass file. Only one styles file is allowed.`)
    }
    cssExtension = '.sass'
