...
let fromFileGroup = (input, group) => {
  ...
  let cssStats = matchingCssFile && safeStatSync(cssInput)
  let htmlStats = matchingHtmlFile && safeStatSync(htmlInput)
  let jsStats = matchingJsFile && safeStatSync(jsInput)
...
let safeStatSync = fileName => {
  if (fileName) {
    try {
      return fs.statSync(fileName)
    } catch (error) {
      fail(`Could not stat ${fileName}, ${error}`)
    }
  }
