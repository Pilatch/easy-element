...
let buildInputFile = (options, reportError) => {
  try {
...
let onChange = (options, reportError, inputIsDirectory, importsToWatch, rebuild) => changedFileName => {
  if (inputIsDirectory) {
    if (options.bundle) {
    ...
    buildInputFile(options, reportError)
  }
}
